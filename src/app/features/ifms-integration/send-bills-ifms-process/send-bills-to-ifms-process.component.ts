...
import { IfmsBillProcessStateService } from './ifms-bill-process-state.service';
...
@Component({
  selector: 'app-send-bills-to-ifms-process',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './send-bills-to-ifms-process.component.html',
  styleUrl: './send-bills-to-ifms-process.component.scss',
})
export class SendBillsToIfmsProcessComponent implements OnInit {
  ...
  schemeCode = '';
  isDDPaymentDisabled = false;
  billTypeLocked = false;
  isGSTAccount = false;         // flag to conditionally render GST fields

  // Budget head modal filters
  searchMajorHead = '';
  searchSubMajorHead = '';
  searchMinorHead = '';
  searchSubHead = '';
  ...
  ngOnInit(): void {
    this.selectedBills = this.processStateService.getSelectedBills();
    if (this.selectedBills.length === 0) {
      this.router.navigate(['/bill-integration']);
      return;
    }
  ...
  searchList(): void {
    const major = this.searchMajorHead.trim().toLowerCase();
    const subMajor = this.searchSubMajorHead.trim().toLowerCase();
    const minor = this.searchMinorHead.trim().toLowerCase();
    const sub = this.searchSubHead.trim().toLowerCase();
    const detail = this.searchDetailsHead.trim().toLowerCase();
    const soe = this.searchSoe.trim().toLowerCase();

    this.ddoGrantHead = this.ddoGrantHeadCopy.filter((item) => {
      return (
        (!major || item.majorHead.toString().toLowerCase().includes(major)) &&
        (!subMajor || item.subMajorHead.toString().toLowerCase().includes(subMajor)) &&
        (!minor || item.minorHead.toString().toLowerCase().includes(minor)) &&
        (!sub || item.subHead.toString().toLowerCase().includes(sub)) &&
        (!detail || item.detailHead.toString().toLowerCase().includes(detail)) &&
        (!soe || (item.soe && item.soe.toString().toLowerCase().includes(soe)))
      );
    });
  }

  selectPayee(payee: any): void {
    this.isGSTAccount = (payee.ifscCode === 'RBIS0GSTPMT');
    if (this.isGSTAccount) {
      ...
      this.ecsForm.get('gstNumber')?.setValue('');
      this.ecsForm.get('gstFirmName')?.setValue('');
    }
  ...
  openGrantSearch(): void {
    ...
    this.searchMajorHead = '';
    this.searchSubMajorHead = '';
    this.searchMinorHead = '';
    this.searchSubHead = '';
    this.searchDetailsHead = '';
    this.searchSoe = '';
          ...
          next: (res) => {
            ...
            if (res && res.success) {
              ...
              Swal.fire({
                ...
                this.processStateService.clear();
