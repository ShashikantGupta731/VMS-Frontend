...
export class IfmsClaimsComponent implements OnInit {
  claimsForm: FormGroup;
  claims: IfmsClaim[] = [];
  filteredClaims: IfmsClaim[] = [];
  ...
  tableColumns: TableColumn[] = [
    ...
    { key: 'ifmsBillNo', label: 'IFMS Bill No', sortable: true },
    { key: 'claimFor', label: 'Claim For', sortable: true },
    { key: 'date', label: 'Date', sortable: true },
    { key: 'amount', label: 'Amount', sortable: true },
    { key: 'subVoucherNo', label: 'Sub Voucher No', sortable: true },
    { key: 'sanctionOrderNo', label: 'Sanction Order No', sortable: true },
    { key: 'ifmsStatus', label: 'IFMS Status', sortable: true },
  ];
  ...
  tableActions: TableAction[] = [
    {
      label: 'View',
      action: (row: IfmsClaim) => this.viewClaim(row),
    },
    {
      label: 'Download',
      action: (row: IfmsClaim) => this.downloadClaim(row),
    },
  ];
  ...
  private setupSearchDebounce(): void {
    this.searchSubject.pipe(
      ...
      switchMap((searchText) => {
        ...
        return this.ifmsClaimsService.searchClaims(searchText, this.getFilterValues());
      }),
  ...
  private applyFilters(): void {
    ...
    this.ifmsClaimsService.getFilteredClaims(filters).subscribe({
      next: (data: { claims: IfmsClaim[]; summary: FilterSummary }) => {
  ...
  private loadClaims(): void {
    this.isLoading = true;
    this.ifmsClaimsService.getClaims().subscribe({
      next: (claims: IfmsClaim[]) => {
        this.claims = claims;
        this.filteredClaims = claims;
        this.totalItems = claims.length;
        this.updatePagination();
  ...
  updateStatusFromIFMS(): void {
    ...
    this.ifmsClaimsService.updateStatusFromIFMS().subscribe({
      next: (response: { message: string }) => {
        ...
        this.loadClaims(); // Reload data
      },
      error: (error: any) => {
        console.error('Update status error:', error);
        this.toastr.error('Failed to update status from IFMS', 'Error');
        this.isLoading = false;
      }
    });
  }
  ...
  private sortClaims(): void {
    this.filteredClaims.sort((a, b) => {
      const aValue = a[this.sortColumn as keyof IfmsClaim];
      const bValue = b[this.sortColumn as keyof IfmsClaim];
